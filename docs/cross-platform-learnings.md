...
## JUCE VBlankDispatcher crashes with Parallels virtual display driver

- **Date**: 2026-03-07
- **Problem**: PlunderTube standalone (ARM64 native build, 899/899 targets) crashes immediately on launch in Parallels Desktop Windows 11 ARM64 VM. Access violation 0xc0000005 at offset 0x8DC43C in `juce::VBlankDispatcher::updateDisplay()`. D3D11 device creation works fine (confirmed with standalone test program creating D3D11 device on "Parallels Display Adapter (WDDM)" with Feature Level 11_0). The crash occurs during JUCE's monitor/VBlank synchronization setup, before any Visage/bgfx rendering code runs.
- **Solution**: Update JUCE from 8.0.8 to 8.0.12. The fix was added in JUCE 8.0.9 in `juce_VBlank_windows.cpp`: (1) Added null check on `output` in `getMonitorFromOutput()` before calling `GetDesc()`. (2) Added handling for `DXGI_ERROR_NOT_CURRENTLY_AVAILABLE` return from `EnumOutputs()` — Parallels' virtual display adapter returns this instead of `DXGI_ERROR_NOT_FOUND`, causing the old code to continue with a null output pointer and crash. Verified: JUCE-Plugin-Starter template standalone launches and stays running on Parallels Windows 11 ARM64 with JUCE 8.0.12.
- **Insight**: JUCE's VBlankDispatcher queries monitor refresh rates via DXGI. Virtual display drivers (like Parallels Display Adapter WDDM) may return `DXGI_ERROR_NOT_CURRENTLY_AVAILABLE` from `IDXGIAdapter::EnumOutputs()` instead of valid outputs. Prior to 8.0.9, JUCE didn't handle this return code. This is NOT a GPU rendering issue — D3D11 works perfectly. Always use JUCE 8.0.9+ for Windows builds that may run in virtual machines. When debugging Windows crashes over SSH without a full debugger, `dumpbin /PDATA` (ARM64 exception tables) + `procdump64a.exe -e -ma -x` (crash dumps) is effective for mapping crash offsets to function names.
- **Files**: JUCE source `modules/juce_gui_basics/native/juce_VBlank_windows.cpp`, `CMakeLists.txt` (JUCE_TAG)
