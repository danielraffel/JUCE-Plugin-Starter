...
| G.5 | Verify GitHub Actions CI passes on all 3 platforms (push to feature branch). | [!] | IN PROGRESS 2026-03-07: CI builds plugin targets (VST3, CLAP, Standalone) successfully on all 3 platforms. Test compilation fails on all platforms due to PluginProcessor placeholder (same as C.4/G.4 — template-only issue, works after init_plugin_project.sh). Fixed CI workflow: added Ninja for Windows, `-k 0` to continue past test errors. Push pending to verify fix. |
