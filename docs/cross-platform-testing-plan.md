...
| A.6 | End-to-end test: PlunderTube standalone launches on Windows, UI renders via D3D11/Visage. | [!] | BLOCKED: UTM VM lacks GPU passthrough — D3D11/WARP fails, standalone exits immediately. Build success confirmed on ARM64 VM (A.2). JUCE-Plugin-Starter CI (G.5) confirms Windows x86-64 builds work. Full Visage rendering test requires real Windows hardware or GPU-enabled environment. |
...
| G.4 | Ubuntu VM: Run Catch2 tests. | [!] | BLOCKED: Same pre-existing issue as C.4 — template test files reference `PluginProcessor` class name which only exists after init_plugin_project.sh replaces placeholders. Catch2 library compiles fine; test source files fail with `use of undeclared identifier 'PluginProcessor'`. Not a platform-specific issue. |
...
| H.4 | Fix Linux-specific compilation errors (guided by H.1 audit). | [!] | IN PROGRESS 2026-03-07: Fixed 4 issues: (1) shaderc Exec format error → VISAGE_BUILD_SHADERC=ON. (2) int64_t/int64 ambiguity in var casts (PluginProcessor, SliceManager) → static_cast<int>/static_cast<juce::int64>. (3) jmax<int64> SIMD error on NEON → std::max. (4) GTK gtk.h not found → JUCE_WEB_BROWSER=0. (5) 0LL vs int64_t type mismatch → int64_t literals. Clean rebuild in progress. |
