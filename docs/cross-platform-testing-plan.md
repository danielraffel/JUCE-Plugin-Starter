...
| H.3 | Clone PlunderTube on Ubuntu VM, attempt CMake configure. | [!] | IN PROGRESS 2026-03-07: Shallow clone (`--depth 1 --branch feature/windows-build`) in progress. Repo is 1.4GB — UTM VM network is slow (239MB/1.4GB after 10+ min). Once clone completes, CMake configure with Ninja + Clang should work (deps verified in G.1/H.2). |
