...
| H.4 | Fix Linux-specific compilation errors (guided by H.1 audit). | [!] | IN PROGRESS 2026-03-07: First build attempt hit shaderc "Exec format error" — pre-built shaderc is x86-64, VM is ARM64. Reconfigured with VISAGE_BUILD_SHADERC=ON to build shaderc from source (same fix as A.1 for Windows). Rebuild in progress. No C++ source compilation errors seen yet — Phase A Windows fixes appear to cover Linux too. |
