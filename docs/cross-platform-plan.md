...
| 2.3 | JuceVisageBridge for Windows (Win32 HWND, D3D11 via Visage/bgfx). | `feature/cross-platform-2-juce-bridge-win` | Visage | [~] | Bridge code compiles and links on Windows ARM64 (PlunderTube Standalone + VST3 build succeeded 2026-03-07). D3D11 rendering not yet visually verified — UTM VM lacks GPU passthrough so may use WARP. Need to launch .exe and confirm UI renders. |
...
| 3.2 | JuceVisageBridge for Linux (X11 window embedding, Vulkan rendering via Visage/bgfx). | `feature/cross-platform-3-juce-bridge-linux` | Visage | [!] | RESEARCHED 2026-03-07: Template bridge (`templates/visage/JuceVisageBridge.cpp`) is already cross-platform — `createPluginWindow()` creates `WindowX11` on Linux automatically. Visage has full X11 windowing in `visage_windowing/linux/windowing_x11.h`. Testing blocked by Ubuntu VM lacking GPU/Vulkan passthrough and sudo password for dependency install. Fixed macOS-specific comments in template to say "desktop" and "GPU" instead of "macOS" and "Metal". |
