...
| 2.3 | JuceVisageBridge for Windows (Win32 HWND, D3D11 via Visage/bgfx). | `feature/cross-platform-2-juce-bridge-win` | Visage | [!] | Bridge code exists in PlunderTube (VisageWindowAdapter, JuceVisageBridge with _WIN32 guards). Windows VM unreachable for verification testing. Needs human intervention: restart Windows VM and run build to confirm D3D11 rendering. |
