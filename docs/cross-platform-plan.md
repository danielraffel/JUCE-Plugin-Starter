...
| 3.2 | JuceVisageBridge for Linux (X11 window embedding, Vulkan rendering via Visage/bgfx). | `feature/cross-platform-3-juce-bridge-linux` | Visage | [!] | Bridge code is platform-agnostic (same VisageWindowAdapter/JuceVisageBridge). No Linux dev environment available for Vulkan rendering verification. Needs human intervention: set up Linux VM or use CI to validate. |
