...
| 2.3 | JuceVisageBridge for Windows (Win32 HWND, D3D11 via Visage/bgfx). | `feature/cross-platform-2-juce-bridge-win` | Visage | [ ] | Unblocked: Visage is fully cross-platform (D3D11 on Windows). PlunderTube Windows port in progress on feature/windows-build. |
...
| 3.2 | JuceVisageBridge for Linux (X11 window embedding, Vulkan rendering via Visage/bgfx). | `feature/cross-platform-3-juce-bridge-linux` | Visage | [ ] | Unblocked: Visage is fully cross-platform (Vulkan on Linux). Needs Linux dev environment for testing. |
